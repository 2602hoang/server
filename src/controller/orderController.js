...
export async function createOrder(req, res) {
  ...
  try {
    ...
    const newOrder = await Order.create(
      { id_user, id_pay, id_adress, notes, status, date_order },
      { transaction }
    );
    // total_price
    let total_price = 0;
    ...
    for (const item of orderItems) {
      ...
      if (id_product) {
        ...
        // console.log(stock);
        itemtotalprice =
          product.price * qty - (product.price * qty * product.discoust) / 100;
    ...
    await newOrder.update(
      { total_price },
      { where: { id_order: newOrder.id_order }, transaction }
    );
...
export async function getAllOrders(req, res) {
  try {
    ...
    const queryParams = {
      ...
      order: [[sequelize.col("date_order"), "DESC"]],
      include: [
        {
          model: OrderDetail,
    ...
    const { rows } = await Order.findAndCountAll(queryParams);
    if (rows.length === 0) {
    ...
    res.status(200).send({
      ...
      order: rows,
      totalItems: countOrder.length,
      totalPages: Math.ceil(countOrder.length / limit),
      currentPage: page,
    });
  } catch (error) {
    console.error(error);
...
// export async function getAllOrders1(req, res) {
//   try {
//     const page = parseInt(req.query.page) || 1;
//     const limit = parseInt(req.query.limit) || 0;
...
export async function getAllOrdersbyIdpay(req, res) {
  try {
    ...
    if (!id_pay) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid pay ID" });
    }
...
export async function addNoteOrder(req, res) {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Note added to order successfully" });
  } catch (error) {
