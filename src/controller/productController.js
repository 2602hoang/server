...
export async function addProduct(req, res) {
  try {
    ...
    if (isNaN(id_brand) || isNaN(id_category)) {
      ...
      return res
        .status(400)
        .json({ success: false, message: "Invalid brand or category ID" });
    }
...
export async function updateProduct(req, res) {
  try {
    const { id_brand, id_category, name, description, price, stock, discoust } =
      req.body;
...
export async function deleteProduct(req, res) {
  ...
  try {
    ...
    const orderDetailExists = await OrderDetail.findOne({
      where: { id_product: parsedId },
      transaction,
    });
